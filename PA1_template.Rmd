...
```{r}
with(data2, plot(interval, steps/480, col = date, xlab = "interval", ylab = "average of steps per day"))
for (i in 1:61)
{
...
>>>>>>> parent of 64d3585... Add

```

<<<<<<< HEAD
Actually, it is a bit of mess in the figure, but that does not affect the highest pionts. We can clearly see them...
...
row.names(Data2) <- c()
library(lattice)
xyplot(Data2$steps ~ Data2$interval |Data2$workday, layout = c(1,1), type = "l", ylab = "Num of steps", xlab = "interval")
...
So I create a new variable workday which has two values: "weekden"and "weekday" as  request. And plot the panel figures as above.
